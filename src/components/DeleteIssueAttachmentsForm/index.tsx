...
const DeleteIssueAttachmentsForm = () => {
  ...
  const deleteAllAttachments = async () => {
    ...
    if (!attachments || !attachments.length) {
      setConsole(`Notice: ${issueIdOrKey} に添付ファイルはありませんでした。🥺`);
      return;
    }
    let log: string[] = [`Notice: ${issueIdOrKey} ======= START =======`];
    for (const a of attachments) {
      const res = await api('DELETE', `/issues/${issueIdOrKey}/attachments/${a.id}`);
      if (!res) {
  ...
  return (
    <Section>
      ...
      <div className={styles.inputs}>
        <TextInput value={issueIdOrKey} onChange={setIssueIdOrKey} placeholder="課題のID または 課題キー" />
